...
### 2026-06-17 (이어서) — 카테고리 상품 진열순서를 원본과 일치
사장님 지적: 카테고리별 상품 등록(진열) 순서가 원본과 다름. 클론은 `categories.json`의 `productIds` 배열 순서대로 렌더하는데, 그 순서가 원본 진열순서와 달랐음(sync의 `ORDER BY DISPLAY_SORT,PRODUCT_ID` ≠ 원본 웹 진열순서).
- **원본 진열순서 스크랩(`scraper/scrape-order.mjs`):** 라이브 `productList?categoryId=ID` + 페이징 `c=pageNo%3DN`(blueshop 방식, GET `pageNo`는 안 먹음)로 각 카테고리의 **전체상품 리스트(largelist4, 페이지당 16개)** productId를 순서대로 수집 → `data/_original-order.json`. (베스트 섹션 4개는 largelist4 밖이라 제외)
- **재정렬(`scraper/reorder-from-original.mjs`):** `[원본순서 ∩ 우리상품]` 먼저, 그 뒤 `[라이브엔 없지만 우리 데이터엔 있는 상품]`을 기존순서로 append. 직접 ID 매칭 우선, 직접ID 없는 카테고리만 수동매핑(X2 기타→원본111). X1 폭포석(1개)은 원본 진열에 없어 유지.
- **결과:** 상품분류 8개 + X2 + 용도별 7개 = 16개 카테고리 재정렬. 동양란 첫5 = 원본과 정확히 일치(050050 백자매화난분-황제…). count 무결성 OK, 상품 238개 전부 보존(productId 집합 불변, 순서만 변경).
- ⚠️ **카탈로그 드리프트:** 과거 sync 시점(우리 238개)과 현재 라이브 카탈로그가 다름. 라이브 동양란 50 vs 우리 75, 서양란 41 vs 68 등(품절/숨김 차이로 추정). 그래서 라이브에 없는 우리 상품(동양란25·서양란27·관엽3·분재5)은 원본순서 뒤에 붙임. 카테고리 헤더 "(전체 N건)"의 N은 우리 카탈로그 기준이라 라이브와 다를 수 있음(순서와 별개 이슈).
- **미적용(별개):** 베스트 섹션(largepd4 상위4)은 원본이 별도 큐레이션(베스트셀러)인데 클론은 리스트 첫 4개를 보여줌 — 진열순서 요청 범위 밖이라 유지. 필요시 best ID도 스크랩해 반영 가능.
- 백업: `data/_backup_categories_*.json`(gitignore 추가). 원본순서 원천: `data/_original-order.json`(보존).

### 2026-06-17 (이어서) — 카탈로그를 "라이브 쇼핑몰" 기준으로 일치 (DB 아님)
사장님 방침: **DB가 아니라 현재 라이브 쇼핑몰과 갯수가 맞아야 함**. DB엔 있지만 쇼핑몰엔 없는 상품은 빼는 게 정상. (베스트·명품은 별도 큐레이션 기능 + 무작위 로테이션이라 순서/멤버십 매칭 대상 아님 — 손대지 않음.)
- **분석:** `_original-order.json`(라이브 스크랩) 기준, 라이브 메뉴 카탈로그 **181개** vs 우리 238개. **라이브엔 있는데 우리가 없는 상품 = 0**(가져올 것 없음, 우리가 데이터는 다 보유). 우리 - 라이브 = 57개 제거 대상.
- **`scraper/match-live-catalog.mjs`:** products.json을 keep-set(181)만 남기고 축소(상품데이터는 보존, 빼기만). categories.json은 라이브 순서/멤버십/카운트로 재작성.
  - 상품분류 갯수 라이브 일치: 동양란 75→50, 서양란 68→41, 관엽 26→23, 분재 25→20, **공기정화 7→10(증가)**, 근조8·축하7·꽃바17·기타4 유지.
  - **폭포석(X1) 분류·상품 제거** — 라이브 메뉴에 없는 DB전용 분류. 라이브110(메뉴에 없는 숨은분류) 전용 7개도 제외.
  - 용도별도 라이브 기준 갱신(203 개업 16→27 등).
- **메뉴 순서도 원본과 일치:** 기타(X2=라이브111)를 꽃바구니 다음(6번째)으로 이동 → 동양란·서양란·근조·축하·꽃바구니·기타·관엽·공기정화·분재.
- 검증: products 181/categories 16, count 무결성 OK, 댕글링 0. /category/X1 → 404, 메뉴에 폭포석 없음. 카테고리 헤더 "전체 N건"이 라이브와 일치.
- 백업: `data/_backup_livematch_*`(gitignore). 되돌리려면 백업 복원.
- 참고(미변경): 베스트/명품 섹션은 무작위 로테이션 기능이라 그대로. SP050025 1개는 용도별에만 속함(라이브 반영, 무해).
